...
// console.log(searchQuerry);
  ...
  return (
    <div className='sticky top-0 z-50 border border-black bg-white grid grid-flow-col px-4 max-sm:pl-2 max-sm:pr-4 shadow-md shadow-gray-500 py-4 max-sm:py-2 '>
      ...
      <div className='flex max-sm:w-fit sm:col-span-1 '>
        ...
        <img className='h-10 px-4 mix-blend-darken max-sm:h-6' alt='youtube logo' src='https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Logo_of_YouTube_%282015-2017%29.svg/2560px-Logo_of_YouTube_%282015-2017%29.svg.png'/>
         </div>

         {/* search input button */}
      <div className=' sm:col-span-10  justify-center flex items-center relative'>

        <input value={searchQuerry} onChange={(e)=>setSearchQuerry(e.target.value)} onFocus={()=>setIsSuggestionsOpen(true)} className='w-2/4 max-sm:w-full max-sm:h-6 px-2 sm:px-4 py-2 max-sm:text-xs max-sm:font-bold font-semibold max-sm:placeholder:text-sm border border-black outline-blue-500 rounded-l-full' placeholder='Search'/> { searchQuerry!==""? <button onClick={()=>setSearchQuerry("")} className='absolute sm:right-80 right-10 sm:bg-slate-400 sm:p-1 rounded-full max-sm:text-[10px]'>❌</button>:""}

        <button onClick={()=>{getFilterVideos(searchQuerry);setIsSuggestionsOpen(false)}} className='w-12 max-sm:h-6 max-sm:w-7 py-2 px-3 max-sm:px-1 max-sm:py-1 rounded-r-full border border-gray-500 bg-gray-200'><img className='h-6 max-sm:h-3 max-sm:w-3' alt='search icon' src='https://upload.wikimedia.org/wikipedia/commons/thumb/0/0b/Search_Icon.svg/1024px-Search_Icon.svg.png'/></button>

        {
          isSuggesionsOpen && <div className='bg-white w-full sm:w-2/4  rounded-xl shadow-lg shadow-gray-600 px-5 mx-1 absolute sm:left-64 top-7 sm:top-11'>
          <ul>
