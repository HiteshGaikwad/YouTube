...
const SuggestionVideoCard = ({video}) => {
  ...
  return (
    <div className='flex max-sm:pb-1 max-sm:my-2 gap-1 max-sm:flex-col shadow-lg shadow-gray-300 rounded-xl'>
      ...
      <div className='max-sm:flex-col w-full max-sm:h-64 flex'>
            <img className='sm:w-48 w-full h-44 sm:h-28 rounded-xl' alt='thumbnail' src={video?.snippet?.thumbnails?.medium?.url}/>
            
             <div className='flex flex-col gap-2 m-1'>
            <h1 className='text-md font-bold overflow-hidden sm:w-full w-[90%] line-clamp-2'>{video?.snippet?.title===undefined?"Video title":video?.snippet?.title}</h1>
        ...
        <button className='max-sm:absolute max-sm:-top-[80px] max-sm:right-2 hover:bg-slate-300 shadow-sm shadow-gray-600 rounded-xl h-9 px-1 w-fit'><img className='w-5 h-5  mt-2' alt='options icon' src='https://static.thenounproject.com/png/1409295-200.png'/></button>
